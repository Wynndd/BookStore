...
public class UserDaoImpl extends BaseDao implements UserDao {
    ...
    public User querybyUsername(String username) {
        String sql = "select `id`,`username`,`password`,`email`,`address`,`balance` from t_user where username = ?";
    ...
    public int updateUser(User user) {
        String sql = "update t_user set `username`=?,`password`=?,`email`=?,`address`=?,`balance`=? where id=?";
        return update(sql,user.getUsername(),user.getPassword(),user.getEmail(),user.getAddress(),user.getBalance(),user.getId());
    }
